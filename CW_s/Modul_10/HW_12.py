...
class Phone(Field):
    ...
    def phone_verification(self, phone):
...
class AddressBook(UserDict):
    ...
    def save_file(self):
        with open('AdressBookData.bin', 'wb') as s_file:
            pickle.dump(self.data, s_file)

    def load_file(self):
        try:
            with open('AdressBookData.bin', 'rb') as l_file:
...
def phone(string):
    new_elem = string.split()
    return users.get_contact(new_elem[0])
